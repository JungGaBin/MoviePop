...
for i in range(4,1317):
  idx = 'B' + str(i)
  r  = requests.get("https://search.naver.com/search.naver?query=영화+" + sheet[idx].value)
  ...
  try:
    ...
    r2  = requests.get("https://search.naver.com/search.naver?where=news&query=영화+" + urllib.parse.quote(sheet[idx].value) + "&ie=utf8&pd=3&ds=" + year + "." + month + "." + sheet['K' + str(i)].value + "&de=" + sheet['H' + str(i)].value.replace('-','.'))
    soup2 = BeautifulSoup(r2.text)
